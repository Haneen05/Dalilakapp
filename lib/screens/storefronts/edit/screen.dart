...
class EditStorefrontScreen extends StatefulWidget {
  ...
  const EditStorefrontScreen(this.id, {super.key});
...
class _EditStorefrontScreenState extends State<EditStorefrontScreen> {
  ...
  var storefront = Completer<Storefront>().future;

  @override
  void initState() {
    storefront = api.storefronts.findOne(widget.id);

    storefront.then((value) async {
      generalInfoKey.currentState!.mapFrom(value);
      contactInfoKey.currentState!.mapFrom(value);
      operatingHoursKey.currentState!.mapFrom(value);
      featuredKey.currentState!.mapFrom(value);
      portfolioImagesKey.currentState!.setState(() {
        portfolioImagesKey.currentState!.images.addAll(
          value.portfolioImages?.map((url) => XFile(url)) ?? [],
        );
      });
    });
      ...
      child: ConstrainedBox(
        ...
        child: Container(
          ...
          child: DColumn(
            mainAxisSpacing: 8.0,
            children: [
              const TopBarWidget(title: Text('Edit Storefront')),
              DColumn(
  ...
  Future<void> editStorefront() async {
    ...
    try {
      ...
      await api.storefronts.update(
        ...
        removeImage: generalInfo.input.existingImage == null,
      );

      if (mounted) {
        Navigator.of(context).popUntil((route) =>
            route.isFirst || route.settings.name == '/dashboard/storefronts');

        if (ModalRoute.of(context)?.settings.name != '/dashboard/storefronts') {
  ...
  Future<void> confirmThenDelete() async {
    bool confirmed = await showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('DeleteStoreFront'),
          content:
              const Text('AreYouSureYouWantToDeleteThisStoreFront?'),
          actions: [
            TextButton(
              onPressed: () {
    ...
    if (confirmed) {
      await api.storefronts.delete(widget.id);
